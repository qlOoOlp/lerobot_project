...
**BYO Policy 규칙** (`lerobot_policy_umidiffusion`):
- config `@PreTrainedConfig.register_subclass("umidiffusion")`, `UmiDiffusionConfig(DiffusionConfig)`
- class `UmiDiffusionPolicy(DiffusionPolicy)`, `name="umidiffusion"`
...
**lerobot_hong 구조 그대로 따름** (§7.2/§9.5: `steps.py`=순수 로직 / `processor_umidiffusion.py`=조립):
```python
...
1. [x] **2-1 정책 껍데기** ✅ (2026-07-17) — `configuration_umidiffusion.py`(**`register_subclass` 만**, 필드는 선언만) + `modeling_umidiffusion.py`(4줄)
   - 배우는 것: **BYO Policy 플러그인 규칙**, 왜 factory 패치가 불필요한지(`_get_policy_cls_from_policy_name` 폴백이 **이름 컨벤션**으로 찾음)
   - `__init__.py` export 가 **등록 트리거** — 빼면 조용히 실패 (dev_plan §12.3)
   - ✅ **결과**: `UmiDiffusionPolicy` **263.2M params** / input `{rgb (3,240,240), state (10,)}` / output `{action (10,)}`
     — **0-patch 첫 실증**(lerobot_hong 은 `policies/factory.py` 에 +45줄 필요했음). `lerobot-train --policy.type=umidiffusion` **완주**.
   - ★ **여기서 패키지화 문제가 터졌다** — 설치된 배포판이 `custom/...` 경로를 import → `ModuleNotFoundError` →
