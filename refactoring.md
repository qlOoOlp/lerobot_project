...
3. [ ] **수집/변환 → LeRobotDataset**: `collect_metaworld_canonical.py`(in-env 수집, **권장** — per-step dynamics 가 rollout 과 일치) 또는 `convert_metaworld_canonical.py`(mt50 변환). 수동 features(canonical 10D) + `LeRobotDataset.create` + `add_frame` 루프 + `save_episode` (**port_droid 패턴, Robot 없음**)
   - ★ **그리퍼 = `obs[3]` 을 threshold 로 이진화** → `state4_to_canonical10(state4, gripper_threshold)` 가 내부 처리. threshold 는 **config 로 주입**(pick-place `0.7`), 하드코딩 금지
   - **시프트 없음**: `obs[3]` 이 진짜 상태라 `action[t] = state[t+1]` 규칙 그대로
   - threshold 는 **rollout(Phase 5)에 같은 값**을 넘겨야 함 (train==inference 계약)
