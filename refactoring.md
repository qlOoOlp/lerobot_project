...
| **2** | 정책 + observer | 표현 codec → 런타임 relative step → mypolicy config/modeling → processor 조립 (+depth 게이트) | 무수정 |
...
| **코드** | `custom/common/lerobot_ext_core/` · `custom/envs/metaworld/{canonical,collect}.py` |
...
### 세부 작업
1. [x] **ext_core (최소)** ✅: `common/lerobot_ext_core/` = `keys.py` + `schemas/canonical_ee10.py`
   ...
   - `schemas/` = **표현별 모듈 하나씩** 두는 자리 (`__init__.py` 에 규약 문서화, **재노출 없음** → 어떤 모듈도 "the schema" 행세 못 함)
     ...
     - 다운스트림은 **모듈 import** 로 표현만 갈아끼움: `from custom.common.lerobot_ext_core.schemas import canonical_ee10 as sch` → `sch.STATE_DIM`
...
2. [x] **metaworld env 어댑터** ✅: `custom/envs/metaworld/canonical.py`
   ...
   - "공유 요구가 있는 건 여기, 수집 전용은 collect.py" 가 배치 기준 (`render_frame` 을 collect.py 에서 여기로 옮긴 이유)
   - env 사실은 여기: `STATE4_DIM=4`, `ENV_ACTION_DIM=4`, `ENV_XYZ_SCALE=0.01`, `PICK_PLACE_GRIPPER_THRESHOLD=0.7` / 표현 사실은 ext_core 에서 import (`sch.STATE_DIM` 등, `10`/`9` 하드코딩 0)
...
3. [x] **수집 → LeRobotDataset** ✅: `custom/envs/metaworld/collect.py` (**port_droid 패턴, Robot 없음**)
...
**목표**: mypolicy 를 BYO Policy 규칙대로 만들고, **런타임 anchor-relative 변환까지** 붙여 `pre→forward→post` end-to-end. lerobot 무수정(등록=플러그인 폴백, depth=config 게이트).
...
- `__init__.py` 에서 `MyPolicyConfig`/`MyPolicy`/`make_mypolicy_pre_post_processors` **export**(dev_plan §12.3) — import 시 등록+노출이 함께 일어나게

**lerobot_hong 구조 그대로 따름** (§7.2/§9.5: `steps.py`=순수 로직 / `processor_mypolicy.py`=조립):
```python
...
### 세부 작업 — **의존성 순서** (a→b→c→d)
1. [ ] **2-a 표현 codec**: `schemas/canonical_ee10_se3.py` — `rot6d↔R`, `pose9d↔transform`, `invert_transform`, `relative_transform` (torch, 배치 `...` 지원)
   - **표현 옆에 두는 이유·lerobot_hong 중복 증거**: 부록 D.5
   - ★ **의존성 0으로 단독 검증 가능**: 역변환 왕복(`R→rot6d→R`), `invert @ T = I`, **좌표계 불변성**(`(T@a)⁻¹@(T@b) == a⁻¹@b`)
2. [ ] **2-b 런타임 step**: `custom/processors/robot_maps/.../steps.py` — codec 위에 2개
   - `CanonicalPoseToRelativeObservationStep`: 앵커=`state[:,-1]` → 히스토리를 `anchor⁻¹@state` 로
   - `CanonicalPoseToActionPoseReprStep(action_pose_repr)`: 같은 앵커로 `relative` | 프레임간 `delta`
   - **pose 9D 만 변환, gripper 1D 는 그대로** 이어붙임 / **차원 유지** 10D→10D (§9.4)
   - ⚠ **`action` 없으면 skip** — eval/추론 raw 관측엔 action 이 없음 (§9.3)
   - `@ProcessorStepRegistry.register` + `ProcessorStep` 6메서드 규약
3. [ ] **2-c 정책**: `configuration_mypolicy.py`(register_subclass, `obs_pose_repr`/`action_pose_repr`/`use_depth` + `apply_depth_gate()`) · `modeling_mypolicy.py`(`MyPolicyPolicy(DiffusionPolicy)`, `super()` 전 `apply_depth_gate()`)
   - `normalization_mapping`: `VISUAL=MEAN_STD`, **`STATE=IDENTITY`, `ACTION=IDENTITY`** (근거: dev_plan §11 — canonical stats 로 relative 를 정규화하면 표현공간 불일치. 부수효과로 **metaworld rot6d std=0 나눗셈도 회피**)
4. [ ] **2-d 조립 + 역변환**: `processor_mypolicy.py` — 위 step 들을 pipeline 에 끼움 + `DropObservationKeysProcessorStep`(depth 게이트의 런타임 절반)
   - ★ **`decode_policy_action(action, anchor_state, action_pose_repr)`** — **정책 relative 출력 → 절대 canonical** 역변환. `relative`: `base @ action` / `delta`: 누적 적분
   - **파이프라인 밖에 둔다**: `policy_post` 는 `PolicyAction` 만 받아 **앵커(현재 관측)에 접근 불가** → 원본 UMI(`get_real_umi_action`)·lerobot_hong(`decode_policy_action`, 호출부 5곳) 모두 추론 루프가 직접 호출
   - ⚠ **없으면 Phase 5 가 통째로 틀린 명령을 냄** (relative 를 절대로 오해)
   - **forward/backward 가 같은 `action_pose_repr` + 같은 앵커**(`state[:, -1]` = 현재 관측)여야 정확한 역함수 → train==inference
5. [ ] 설치: `pip install -e` (codec/steps/policy 패키지). **코드 수정은 재설치 불필요**(editable), `pyproject.toml` 변경 시에만 재설치
6. [ ] **검증**: `make_policy(type="mypolicy", ds_meta=...)` → 더미 forward → `pre(sample)→select_action→post(action)` end-to-end + `use_depth` on/off
...
2. [ ] `lerobot-train --policy.type=mypolicy --dataset...`
   - ⚠ **정규화 주의**: `rot6d` 6채널은 **std=0**(Sawyer 무회전) → 정규화 시 0-나눗셈. lerobot Normalizer 가 처리하는지 확인 필요(Phase 1 실측 확인됨)
...
3. [ ] rollout 루프: `env.step()→env_proc→policy_pre→policy→policy_post→**decode_policy_action**→env_proc→env.step()`
   ...
   - 참고: lerobot_hong `rollout_metaworld_mypolicy.py:98` 이 정확히 이 형태 — 그대로 따름
...
| 런타임 relative step (`robot_maps/steps.py`) | **2** (정책의 존재 이유 — dev_plan §12.1) |
...
```python
# umi2lerobot_delta_pose/se3.py (offline, numpy)
def relative_transform(base, target):  return np.linalg.inv(base) @ target   # 일반 역행렬

# robot_maps/steps.py (online, torch)
...
| anchor 의미, relative/delta, Step 래퍼 | `robot_maps/steps.py` | **런타임 윈도우 의미** = 런타임 전용 |
