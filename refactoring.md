...
### 세부 작업
1. [x] train config ✅ — `use_depth=False`, dataset root = **`~/datasets/metaworld_canonical/pick_place_v4`** (300ep/16,420프레임, 시딩 수정 후 재수집분)
   - `delta_timestamps` 는 config 의 `observation_delta_indices`/`action_delta_indices` 가 **자동 생성** — 실측: obs `[-0.0125, 0.0]`(2프레임 @80fps) / action 16프레임
2. [x] `lerobot-train --policy.type=umidiffusion --dataset...` ✅
...
3. [x] **loss 하강 검증** ✅ — 600스텝(1 epoch) `0.958 → 0.032`, grad norm 3.07 → 1.17 (발산 아니라 안정화)
   - 2-0 의 기본 diffusion(200스텝 0.843)보다 **빨리 내려간다** — anchor-relative 가 월드 좌표 암기를 없애 문제를 쉽게 만든 것으로 보임
4. [x] **체크포인트 저장/로드 검증** ✅ — ★ 여기가 Phase 4 의 진짜 항목이었다. 아래 참조

> ### ★ 체크포인트 왕복 계약 — Phase 5 가 반드시 지켜야 할 것 (2026-07-17)
>
> 저장된 `policy_preprocessor.json` 이 우리 step 을 이렇게 기록한다:
> ```
> [3] canonical_pose_to_action_pose_repr   config={'action_pose_repr':'relative', 'state_key':'observation.state'}
> [4] canonical_pose_to_relative_observation config={'state_key':'observation.state'}
> [1] lerobot_policy_umidiffusion.processor_umidiffusion.DropObservationKeysProcessorStep  config={'keys':[...]}
> ```
> **비대칭 주의**: `steps.py` 의 2개는 `@ProcessorStepRegistry.register` 가 붙어 **레지스트리 이름**으로,
> `DropObservationKeysProcessorStep` 은 등록이 없어 **전체 클래스 경로**로 저장된다. 둘 다 복원되지만
> 경로가 다르다 — 클래스를 옮기면 후자의 옛 체크포인트가 깨진다.
>
> **★ 로드하려면 플러그인이 먼저 import 돼야 한다** — 등록은 import 시점에 일어나므로:
> ```python
> PreTrainedConfig.from_pretrained(path)     # 그냥 하면 → KeyError: 'umidiffusion'  ← 실측
>
> # 둘 중 하나가 반드시 선행:
> import lerobot_policy_umidiffusion                        # 명시적 import
> register_third_party_plugins()                            # lerobot 정석 (CLI 진입점이 하는 것)
> ```
> `lerobot-train`/`lerobot-eval` 은 스스로 `register_third_party_plugins()` 를 부르지만,
> **우리가 직접 짜는 Phase 5 rollout 스크립트는 안 부른다** → 빠뜨리면 `KeyError` 로 죽는다.
> (죽는 건 다행이다 — 오늘 만난 다른 실패들과 달리 **조용하지 않다**.)
>
> ✅ **검증**: import 후 로드 시 step 7개 전부 복원 · **순서 유지**(action idx3 < obs idx4) ·
> `action_pose_repr='relative'` / `state_key` / Drop 의 `keys=('observation.images.depth',)` 까지 정확히 복원.

### 증강 (선택) — 두 갈래이고 층이 다르다
```
수집:  env → flip/resize → 데이터셋            ← 고정·결정적
학습:  데이터셋 → ★증강★ → pre → 정책          ← 증강은 여기만
추론:  env → flip/resize → pre → 정책          ← 증강 없음. 수집과 '같은 함수'
```
`train == inference` 는 **"데이터셋 == 추론 입력"** 이고, 증강은 그 위에 얹는 **의도적 학습 노이즈**다.

| 갈래 | 켜는 법 | 성격 |
|---|---|---|
| **정책 crop** (DP 논문의 주력 증강) | `--policy.crop_shape='[216,216]'` | `DiffusionRgbEncoder` 가 **학습=RandomCrop / 추론=CenterCrop** 으로 자동 분기. lerobot 이 이미 구현. **우리는 `crop_shape=None` 이라 꺼져 있다** |
| **데이터셋 광도** | `--dataset.image_transforms.enable=true` | brightness/contrast/saturation/hue/sharpness. 픽셀 값만 → 안전 |
| **데이터셋 affine** ⚠ | 위와 같은 플래그에 포함(`RandomAffine ±5°, translate 5%`) | **기하 변형 — 이미지만 틀고 action 라벨은 그대로** → 라벨 노이즈. 우리 정책은 이미지로 물체 위치를 추론하므로 **권하지 않음** |

> 지금은 **증강 없이** 수렴을 확인했다(600스텝 0.032). 일반화가 부족하면 **crop 부터** 켤 것.
