"""UmiDiffusion config — DiffusionConfig + pose 표현 옵션 + depth 게이트.

■ 플러그인 등록 (lerobot 무수정)
    @PreTrainedConfig.register_subclass("umidiffusion") 만 붙이면
    lerobot 의 _get_policy_cls_from_policy_name 폴백이 `lerobot_policy_umidiffusion` 를
...
class UmiDiffusionConfig(DiffusionConfig):
    ...
    def __post_init__(self) -> None:
      ...
      if self.obs_pose_repr != "relative":
        raise ValueError(f'`obs_pose_repr` must be "relative" (the only supported value). Got {self.obs_pose_repr}.')
      if self.action_pose_repr not in {"relative", "delta"}:
        raise ValueError(f'`action_pose_repr` must be one of {{"relative", "delta"}}. Got {self.action_pose_repr}.')
