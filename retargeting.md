...
| **lerobot_hong** | `decode_policy_action` — 수식 동일, 호출부 5곳(`run_mypolicy_inference`/`_ros2`/`offline_`/**`rollout_metaworld_`**/smoke) |
...
**부수효과**: metaworld 의 **rot6d std=0** 나눗셈 문제도 함께 회피됨. 필요해지면 relative 기준 stats 를 따로 계산하는 방향으로 확장.
